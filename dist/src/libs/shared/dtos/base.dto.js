"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BaseDto = void 0;
const openapi = require("@nestjs/swagger");
const class_transformer_1 = require("class-transformer");
const account_basic_dto_1 = require("./account-basic.dto");
const basic_dto_1 = require("./basic.dto");
let BaseDto = class BaseDto extends basic_dto_1.BasicDto {
    static _OPENAPI_METADATA_FACTORY() {
        return { createdAt: { required: false, type: () => Date }, updatedAt: { required: true, type: () => Date }, createdBy: { required: false, type: () => require("./account-basic.dto").AccountBasicDto }, updatedBy: { required: true, type: () => require("./account-basic.dto").AccountBasicDto } };
    }
};
exports.BaseDto = BaseDto;
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_transformer_1.Transform)(({ value, obj }) => {
        if (obj.createdBy) {
            return value;
        }
        else {
            return undefined;
        }
    }, { toClassOnly: true }),
    (0, class_transformer_1.Type)(() => Date),
    __metadata("design:type", Date)
], BaseDto.prototype, "createdAt", void 0);
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_transformer_1.Type)(() => Date),
    __metadata("design:type", Date)
], BaseDto.prototype, "updatedAt", void 0);
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_transformer_1.Type)(() => account_basic_dto_1.AccountBasicDto),
    __metadata("design:type", account_basic_dto_1.AccountBasicDto)
], BaseDto.prototype, "createdBy", void 0);
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_transformer_1.Type)(() => account_basic_dto_1.AccountBasicDto),
    __metadata("design:type", account_basic_dto_1.AccountBasicDto)
], BaseDto.prototype, "updatedBy", void 0);
exports.BaseDto = BaseDto = __decorate([
    (0, class_transformer_1.Exclude)()
], BaseDto);
//# sourceMappingURL=base.dto.js.map